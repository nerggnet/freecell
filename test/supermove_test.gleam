...
pub fn capacity_is_one_more_than_the_free_cells_test() {
  assert rules.capacity(rules.Standard, packed(), Cascade(0)) == 5

  let three = fixture.with_cells(packed(), ["2C", "", "", ""])
  assert rules.capacity(rules.Standard, three, Cascade(0)) == 4

  let none = fixture.with_cells(packed(), ["2C", "2D", "2H", "2S"])
  assert rules.capacity(rules.Standard, none, Cascade(0)) == 1
}
...
pub fn empty_columns_double_the_capacity_test() {
  ...
  assert rules.capacity(rules.Standard, one_gap, Cascade(0)) == 10

  let two_gaps =
    fixture.board_from(["KS", "KH", "KD", "KC", "QS", "QH", "", ""])
  assert rules.capacity(rules.Standard, two_gaps, Cascade(0)) == 20
}
...
pub fn the_destination_column_does_not_count_towards_its_own_capacity_test() {
  let one_gap =
    fixture.board_from(["KS", "KH", "KD", "KC", "QS", "QH", "QD", ""])
  assert rules.capacity(rules.Standard, one_gap, Cascade(7)) == 5
  assert rules.capacity(rules.Standard, one_gap, Cascade(0)) == 10
}
...
pub fn a_run_moves_onto_a_matching_anchor_test() {
  ...
  let assert Ok(after) =
    rules.apply_run(rules.Standard, board, Move(Cascade(0), Cascade(1)), 3)
...
pub fn a_run_is_refused_when_its_anchor_does_not_fit_test() {
  ...
  assert rules.apply_run(rules.Standard, board, Move(Cascade(0), Cascade(1)), 3)
...
pub fn only_a_real_sequence_can_move_as_one_test() {
  ...
  assert rules.apply_run(rules.Standard, board, Move(Cascade(0), Cascade(1)), 9)
    == Error(NotASequence)
}
...
pub fn a_run_is_refused_when_there_is_nowhere_to_stage_it_test() {
  ...
  assert rules.capacity(rules.Standard, board, Cascade(1)) == 1
  assert rules.apply_run(rules.Standard, board, Move(Cascade(0), Cascade(1)), 3)
    == Error(NotEnoughRoom(1))

  // A single card never needs staging, so it still moves where it fits.
  let assert Ok(_) =
    rules.apply_run(rules.Standard, board, Move(Cascade(0), Cascade(2)), 1)
}
...
pub fn a_blocked_run_reports_the_lack_of_room_test() {
  ...
  assert rules.capacity(rules.Standard, board, Cascade(1)) == 2
  assert rules.apply_run(rules.Standard, board, Move(Cascade(0), Cascade(1)), 3)
    == Error(NotEnoughRoom(2))
  assert rules.longest_run(rules.Standard, board, Move(Cascade(0), Cascade(1)))
    == Error(NotEnoughRoom(2))
}

pub fn runs_only_move_between_cascades_test() {
  let board = fixture.board_from(["7H 6S 5D", "", "", "", "", "", "", ""])
  assert rules.apply_run(rules.Standard, board, Move(Cascade(0), Free(0)), 3)
    == Error(RunsOnlyBetweenCascades)
  assert rules.apply_run(rules.Standard, board, Move(Cascade(0), Cascade(0)), 3)
    == Error(rules.SameLocation)
}

pub fn moving_no_cards_is_not_a_move_test() {
  let board = fixture.board_from(["7H 6S 5D", "8S", "", "", "", "", "", ""])
  assert rules.apply_run(rules.Standard, board, Move(Cascade(0), Cascade(1)), 0)
    == Error(NoCardsToMove)
  assert rules.apply_run(
      rules.Standard,
      board,
...
pub fn apply_best_takes_as_many_cards_as_will_go_test() {
  ...
  let assert Ok(#(after, moved)) =
    rules.apply_best(rules.Standard, board, Move(Cascade(0), Cascade(1)))
  assert moved == 3
  assert fixture.columns(after) == ["", "8S 7H 6S 5D", "", "", "", "", "", ""]
}
...
/// With every cell full and nowhere empty the standard rules carry one card.
/// The relaxed rules carry the run regardless.
pub fn relaxed_rules_carry_a_run_with_nowhere_to_stage_it_test() {
  let board =
    fixture.board_from(["7H 6S 5D", "8S", "KS", "KH", "KD", "KC", "QS", "QH"])
    |> fixture.with_cells(["2C", "2D", "2H", "2S"])
  ...
  let assert Ok(after) =
    rules.apply_run(rules.Relaxed, board, Move(Cascade(0), Cascade(1)), 3)
  assert fixture.columns(after) |> list.take(2) == ["", "8S 7H 6S 5D"]
}

/// Only the staging limit is lifted. The cards must still be a run, and must
/// still land somewhere they belong — otherwise it would be a different game.
pub fn relaxed_rules_still_want_a_run_and_a_place_to_land_test() {
  let broken = fixture.board_from(["7H 6S 5S", "8S", "", "", "", "", "", ""])
  assert rules.apply_run(rules.Relaxed, broken, Move(Cascade(0), Cascade(1)), 3)
    == Error(NotASequence)

  let mismatched =
    fixture.board_from(["7H 6S 5D", "8H", "", "", "", "", "", ""])
  assert rules.apply_run(
      rules.Relaxed,
      mismatched,
      Move(Cascade(0), Cascade(1)),
...
pub fn relaxed_capacity_is_never_the_limit_test() {
  let cramped =
    fixture.board_from(["KS", "KH", "KD", "KC", "QS", "QH", "QD", "QC"])
    |> fixture.with_cells(["2C", "2D", "2H", "2S"])
  assert rules.capacity(rules.Standard, cramped, Cascade(0)) == 1
  assert rules.capacity(rules.Relaxed, cramped, Cascade(0)) >= 13
}
